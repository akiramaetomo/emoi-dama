...
import { renderOptions } from "./form-renderers";
import { issuerLabels, type HappyBall, type NameBookEntry, type NameRole } from "./models";
import type { AppSettings, BackgroundTexture, EmotionEchoStrength } from "./settings";
...
export function renderToolsPanel(context: ToolsPanelRenderContext): string {
  ...
  return `
    <div class="tools-panel">
      ...
      <details class="settings-group export-panel"${renderDetailsOpen(context, "export-panel")}>
        <summary class="panel-title">
          <h2>バックアップ・復元</h2>
...
export function renderLedgerList(balls: HappyBall[], selectedBallId: string | null): string {
  if (balls.length === 0) {
